...
mvn -X --settings ./travis/maven-settings.xml -U install -DskipTests #> /dev/null
...
if [ "${TRAVIS_PULL_REQUEST}" = "false" ]; then
    ...
    mvn -X --settings ./travis/maven-settings.xml verify -pl '!gws-system-test' -DredirectTestOutputToFile
fi
